...
def lrt_stops
  ...
  execute(<<-SQL)
    ...
    WHERE
      routes.pos = 4 AND routes.company = 'LRT'
  SQL
end
...
def haymarket_and_leith
  ...
  execute(<<-SQL)
    ...
    FROM
      routes
    JOIN
      stops ON stops.id = routes.stops_id
    WHERE
      stops.name = 'Haymarket' OR stops.name = 'Leith'
  SQL
end
...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    JOIN
      stops ON stops.id = routes.stops_id
    WHERE
      stops.name = 'Craiglockhart' AND stops.name = 'Tollcross'
  SQL
end
